  ...
  return (
    <div className="sticky top-8">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-4">Live Summary</h3>

        <div className="space-y-4">
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-sm text-gray-600 mb-1">Allowed Construction Area</p>
            <p className="text-2xl font-bold text-gray-900">{allowedSqft.toLocaleString()} sqft</p>
          </div>

          {currentStep >= 4 && (
            <>
              <div className="bg-blue-50 rounded-lg p-4">
                <p className="text-sm text-blue-600 mb-1">Used Area</p>
                <p className="text-2xl font-bold text-blue-900">{totalSqft.toLocaleString()} sqft</p>
              </div>

              <div className={`rounded-lg p-4 ${remainingSqft >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                <p className={`text-sm mb-1 ${remainingSqft >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  Remaining Space
                </p>
                <p className={`text-2xl font-bold ${remainingSqft >= 0 ? 'text-green-900' : 'text-red-900'}`}>
                  {remainingSqft.toLocaleString()} sqft
                </p>
              </div>

              <div className="bg-purple-50 rounded-lg p-4">
                <p className="text-sm text-purple-600 mb-1">Usage Percentage</p>
                <div className="flex items-end gap-2">
                  <p className="text-2xl font-bold text-purple-900">{usagePercentage.toFixed(1)}%</p>
                </div>
          ...
          {currentStep >= 5 && roofCost > 0 && (
            <div className="bg-amber-50 rounded-lg p-4">
              <p className="text-sm text-amber-600 mb-1">Roof Cost</p>
              <p className="text-2xl font-bold text-amber-900">LKR {roofCost.toLocaleString()}</p>
            </div>
          )}
